...
fill_combined = axes[0].fill_between(
    X, signal(t0, X), 0, where=signal(t0, X) >= 0, color=WHITE, alpha=0.06, zorder=1
)
...
for ax, c in zip(axes[1:], COMPONENTS):
    ...
    fill = ax.fill_between(
        X, y, 0, where=y >= 0, color=c["color"], alpha=0.15, zorder=1
    )
...
def animate(frame):
    ...
    for col in list(axes[0].collections):
        col.remove()
    axes[0].fill_between(
        X, y_combined, 0, where=y_combined >= 0, color=WHITE, alpha=0.06, zorder=1
    )
