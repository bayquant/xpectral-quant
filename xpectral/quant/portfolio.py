...
class Portfolio:
    def __init__(self, df: pl.DataFrame, portfolio: Dict, benchmark: str):
        ...
        _weights_df = (
            pl.DataFrame(
                {"ticker": list(portfolio.keys()), "weight": list(portfolio.values())}
            )
        ).lazy()
        self.assets_df = self.assets_df.join(
            other=_weights_df, on="ticker", how="inner"
        )
    ...
    def compute_returns(self) -> pl.DataFrame:
        ...
        df = df.with_columns(
            [
                (pl.col("systematic_return") * pl.col("weight")).alias(
                    "port_systematic_return"
                ),
